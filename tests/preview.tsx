...
const wait=async(test:()=>boolean,message:string)=>{const end=Date.now()+20000;while(!test()){if(Date.now()>end)throw new Error(message);await new Promise(r=>setTimeout(r,100));}};
function Harness(){
 ...
 async function run(){
  ...
  try{
   await host.create();if(!host.state.inviteUrl)throw new Error(host.state.status);
   ...
   setResult("PASS: real WebRTC video/audio tracks, text, reaction, consented 80KB byte-perfect file transfer, transmitted effect, media toggles and guest refresh reconnect.");
  }catch(e){setResult("FAIL: "+String(e));}finally{setRunning(false);}
 }
